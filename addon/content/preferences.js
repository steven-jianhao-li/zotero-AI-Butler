(function() {
  ...
  try {
    ...
    if (openButton) {
      Zotero.debug('[AI-Butler] Button found. Attaching click listener directly.');
      ...
      openButton.addEventListener('click', function() {
        try {
          ...
          if (addonInstance && typeof addonInstance.hooks?.onOpenMainWindow === 'function') {
            ...
            if (win && win.document) {
              const menuItem = win.document.getElementById('zotero-itemmenu-ai-butler-summary');
              if (menuItem) {
                menuItem.click();
              } else {
    ...
    const addonInstance = Zotero.AIButler;
    if (addonInstance && typeof addonInstance.hooks?.onPrefsEvent === 'function') {
