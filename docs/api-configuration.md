...
> 注意：Ollama 原生接口不支持直接上传 PDF Base64。使用 Ollama 阅读论文时，请在该模型平台详情里的 **PDF 处理方式** 选择 **文本提取** 或 **MinerU**；也可以把全局 PDF 处理模式改为文本提取。AI 管家不会静默改写您选择的 PDF 处理模式；如果仍选择 Base64，会显示明确错误。

---

## PDF 处理模式

AI 管家提供统一的 PDF 输入中间件。您可以设置全局默认 PDF 处理方式，也可以在每个 **模型平台** 的详情里单独覆盖。这样同一套路由中可以让 Gemini/GPT-4o 使用 Base64，让 Ollama、部分 OpenAI-compatible 中转模型使用文本提取或 MinerU。
...
1. 打开 **AI 管家仪表盘** → **快捷设置**
2. 找到 **"全局 PDF 处理模式"** 设置默认行为
3. 如果某个模型需要单独设置，展开该 **模型平台** 的详情，在 **"PDF 处理方式"** 中选择覆盖模式
