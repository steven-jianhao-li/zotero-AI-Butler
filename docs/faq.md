...
新版 AI 管家已将总结、思维导图、填表、文献综述和后续追问统一接入 LLM 输入中间件。切换为文本提取后，这些功能都会使用文本模式；如果仍选择 Base64，插件会按当前 Provider 的 Base64 请求结构发送 PDF，不会静默改成文本模式。若 API 或中转站不兼容，请根据返回错误自行切换文本模式或更换模型。
...
如果您使用的是本地大模型或 OpenAI-compatible 接口，建议长期使用文本提取模式；这样填表和思维导图也不会再尝试上传 Base64 PDF。
