...
function registerPrefsPane() {
  const prefOptions = {
    ...
    scripts: [
      rootURI + `content/scripts/${config.addonRef}-prefs.js`,
    ],
  };
...
async function handleGenerateSummary() {
  ...
  const provider = (Zotero.Prefs.get(`${config.prefsPrefix}.provider`, true) as string) || "openai";
  let apiKey: string | undefined;
  let providerName: string;

  if (provider === "google" || provider === "gemini") {
    apiKey = Zotero.Prefs.get(`${config.prefsPrefix}.geminiApiKey`, true) as string;
    providerName = "Gemini";
  } else {
