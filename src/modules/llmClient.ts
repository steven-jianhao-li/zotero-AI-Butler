...
export class LLMClient {
  ...
  static async generateSummary(
    ...
    const provider = ((getPref("provider" as any) as string) || "openai").trim();
    if (provider === "google" || provider.toLowerCase().includes("gemini")) {
      return await LLMClient.generateWithGemini(content, isBase64, prompt, onProgress);
    }
    
    // OpenAI 模式只支持文本,如果传入Base64则提示错误
    if (isBase64) {
      throw new Error("OpenAI 模式不支持 Base64 PDF 处理,请切换到 Google Gemini 或使用文本提取模式");
    }
    ...
    if (streamEnabled && onProgress) {
      ...
      // 若未拿到任何增量，回退到非流式
      return await LLMClient.nonStreamCompletion(apiUrl, apiKey, basePayload, onProgress);
    } else {
  ...
  private static async generateWithGemini(
    ...
    const baseUrl = ((getPref("geminiApiUrl" as any) as string) || "https://generativelanguage.googleapis.com").replace(/\/$/, "");
    const apiKey = ((getPref("geminiApiKey" as any) as string) || "").trim();
    const model = ((getPref("geminiModel" as any) as string) || "gemini-2.5-pro").trim();
  ...
  private static notifyError(message: string) {
    new ztoolkit.ProgressWindow("AI Butler", { closeOtherProgressWindows: false })
  ...
  static async testConnection(): Promise<string> {
    ...
    if (provider === "google" || provider.toLowerCase().includes("gemini")) {
      ...
      const payload = {
        contents: [{ role: "user", parts: [{ text: "Hello! Please respond with 'OK' to confirm connection." }] }],
      ...
      try {
        ...
        if (response.status === 200) {
          const json = typeof response.response === "string" ? JSON.parse(response.response) : response.response;
        ...
        try {
          const errorResponse = error?.xmlhttp?.response || error?.xmlhttp?.responseText;
          if (errorResponse) {
            const parsed = typeof errorResponse === "string" ? JSON.parse(errorResponse) : errorResponse;
            const err = parsed?.error || parsed;
            const code = err?.code || `HTTP ${error?.xmlhttp?.status || "Unknown"}`;
            const msg = err?.message || "请求失败";
            errorMessage = `${code}: ${msg}`;
          }
        } catch {}
