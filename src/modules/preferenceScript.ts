...
function updatePrefsUI(win: Window) {
  ...
  const apiKeyInput = doc.getElementById("zotero-prefpane-ai-butler-apiKey") as HTMLInputElement | null;
  const apiUrlInput = doc.getElementById("zotero-prefpane-ai-butler-apiUrl") as HTMLInputElement | null;
  const modelInput = doc.getElementById("zotero-prefpane-ai-butler-model") as HTMLInputElement | null;
  const temperatureInput = doc.getElementById("zotero-prefpane-ai-butler-temperature") as HTMLInputElement | null;
  const promptTextarea = doc.getElementById("zotero-prefpane-ai-butler-summaryPrompt") as HTMLTextAreaElement | null;
  const streamCheckbox = doc.getElementById("zotero-prefpane-ai-butler-stream") as HTMLInputElement | null;
...
function diagnosePrefs() {
  ...
  for (const key of keys) {
    try {
      ...
      ztoolkit.log(`  - 是否为空: ${value === undefined || value === null || (typeof value === 'string' && !value.trim())}`);
    } catch (error) {
  ...
  try {
    ...
    win.setTimeout(() => {
      ...
      if (button) {
        ...
        slog("[AI-Butler][Prefs] 可用的元素 ID:", 
          Array.from(doc.querySelectorAll("[id]")).map((el: any) => el.id).join(", "));
      }
      } catch (err) {
        // eslint-disable-next-line no-console
