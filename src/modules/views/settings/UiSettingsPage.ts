...
export class UiSettingsPage {
  ...
  this.container.appendChild(createNotice("界面与行为设置：自动滚动、自动扫描；以及已有 AI 笔记时的处理策略。"));
    ...
    form.appendChild(createFormGroup("自动滚动到最新输出", autoScrollBox, "生成笔记时，自动滚动到输出窗口底部"));

    // 自动扫描
    const autoScan = (getPref("autoScan") as boolean) ?? true;
    const autoScanBox = createCheckbox("autoScan", !!autoScan);
    form.appendChild(createFormGroup("自动扫描新文献", autoScanBox, "监听文献库变化，新加入的文献自动加入分析队列"));

    // 笔记管理策略
  const policy = ((getPref("noteStrategy" as any) as string) || "skip").toString();
    const policySelect = createSelect("notePolicy", [
      { value: "skip", label: "跳过(默认)" },
      { value: "overwrite", label: "覆盖" },
      { value: "append", label: "追加" },
    ], policy);
    form.appendChild(createFormGroup("已有 AI 笔记时的策略", policySelect, "当检测到条目已有 AI 总结笔记时该如何处理"));
  ...
  const btnSave = createStyledButton("💾 保存设置", "#4caf50");
    btnSave.addEventListener("click", () => {
      const autoVal = (form.querySelector('#setting-autoScroll') as HTMLInputElement)?.checked ?? true;
      const autoScanVal = (form.querySelector('#setting-autoScan') as HTMLInputElement)?.checked ?? true;
  const policyVal = (policySelect as any).getValue ? (policySelect as any).getValue() : policy;
  ...
  setPref("noteStrategy" as any, policyVal);
      ...
      new ztoolkit.ProgressWindow("界面设置").createLine({ text: "✅ 设置已保存", type: "success" }).show();
    });
  ...
  setPref("noteStrategy" as any, "skip");
      AutoScanManager.getInstance().reload();
      this.render();
      new ztoolkit.ProgressWindow("界面设置").createLine({ text: "已重置为默认", type: "success" }).show();
    });
